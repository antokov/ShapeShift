...
def get_routine(routine_id: str, user_id: str = "admin") -> dict | None:
    with get_connection() as conn:
        row = conn.execute(
            "SELECT * FROM routines WHERE id = ? AND user_id = ?", (routine_id, user_id)
        ).fetchone()
...
def create_routine(routine: dict, user_id: str = "admin") -> dict:
    with get_connection() as conn:
        conn.execute(
            "INSERT INTO routines (id, name, description, exercises, created_at, routine_type, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (
                routine["id"],
                routine["name"],
...
def update_routine(routine_id: str, routine: dict, user_id: str = "admin") -> dict | None:
    ...
    with get_connection() as conn:
        conn.execute(
            "UPDATE routines SET name=?, description=?, exercises=?, created_at=?, routine_type=? WHERE id=? AND user_id=?",
            (
                routine["name"],
                routine.get("description", ""),
                json.dumps(routine.get("exercises", [])),
...
def workout_row_to_dict(row: sqlite3.Row) -> dict:
    d = dict(row)
...
def get_all_workouts(user_id: str = "admin") -> list[dict]:
    with get_connection() as conn:
        rows = conn.execute(
            "SELECT * FROM workouts WHERE user_id = ? ORDER BY started_at DESC", (user_id,)
        ).fetchall()
...
def get_calendar_events(user_id: str = "admin") -> list[dict]:
    with get_connection() as conn:
        rows = conn.execute(
            "SELECT * FROM calendar_events WHERE user_id = ? ORDER BY rowid ASC", (user_id,)
        ).fetchall()
    result = []
    for r in rows:
        d = dict(r)
...
def add_calendar_event(event: dict, user_id: str = "admin") -> dict:
    with get_connection() as conn:
        conn.execute(
            "INSERT INTO calendar_events(id, routine_id, routine_name, event_type, date, start_date, recurrence_days, user_id) "
            "VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
            (
                event["id"],
                event["routineId"],
...
def delete_calendar_event(event_id: str, user_id: str = "admin") -> bool:
    with get_connection() as conn:
        row = conn.execute(
            "SELECT id FROM calendar_events WHERE id = ? AND user_id = ?", (event_id, user_id)
        ).fetchone()
        if not row:
            return False
